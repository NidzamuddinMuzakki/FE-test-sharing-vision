...
const EditProduct = () => {
  const [log, setSetLog] =  useState<any[]>([])
  const [err, setError] =  useState<any[]>([])
  const [title, setTitle] = useState('')
  const [content, setContent] = useState('')
  const [category, setCategory] = useState('')
  useEffect(()=>{
    if(window.location.pathname.split("/")[window.location.pathname.split("/").length-1]){ 
      axios.get(`http://localhost:8080/v1/article/detail/${window.location.pathname.split("/")[window.location.pathname.split("/").length-1]}`).then(res=>{
       if(res?.data?.data){
      ...
      axios.get(`http://localhost:8080/v1/article/log/${window.location.pathname.split("/")[window.location.pathname.split("/").length-1]}`).then(res=>{
        if(res?.data?.data){
          setSetLog(res?.data?.data)
        }
       }).catch(err=>{
  ...
  const handleSave = (status:string)=>{
    axios.put(`http://localhost:8080/v1/article/${window.location.pathname.split("/")[window.location.pathname.split("/").length-1]}`,{title:title,content:content,category:category,status:status}).then(res=>{
        ...
        <div className="dark:bg-blackPrimary bg-whiteSecondary py-10">
          ...
          <div className="px-4 sm:px-6 lg:px-8 pb-8 pt-8 grid grid-cols-1 gap-x-10 max-xl:grid-cols-1 max-xl:gap-y-10">
            ...
            <div style={{marginTop:'10px'}}>
              <span style={{fontSize:'30px'}}>Logs</span>
              {log.map((data,inde)=>{
                return(
                  <div key={inde} style={{marginTop:'16px'}}>{JSON.stringify(data)}</div>
                )
              })}
            </div>
